...
pipeline {
  ...
  stages {
    stage('Cloning Git') {
            steps {
              ...
              git credentialsId: 'GIT_CREDENTIALS', url:  'https://github.com/agunuworld4/jjva-mss-mdb-springboot-app.git',branch: 'lab-master-branch'
  ...
  stage('SonarQubeReport') {
      steps{
        sh "mvn clean clean package sonar:sonar -Dsonar.projectKey=jjva-mss-mdb-springboot-app -Dsonar.projectName='jjva-mss-mdb-springboot-app' -Dsonar.host.url=http://${sonar_IP_address}:9000 -Dsonar.token=${jjva_mdb_sonar_token}"
           }
        }
  ...
  //     }
    ...
    stage('springboot Deployment') {
            steps {
              parallel(
                "Deployment": {
                     sh 'bash jjva-springboot-script.sh'
                    },
                    "Rollout Status": {
                      sh 'bash jjva-springboot-fallback.sh'
                        },
                        "mongodb commands": {
                       sh 'bash jjva-mdb-script.sh'
                       }
