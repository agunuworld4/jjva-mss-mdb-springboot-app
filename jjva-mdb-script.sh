...
if [[ $? -ne 0 ]]; then
    ...
    kubectl -n jjva-ns-svc-pod apply -f jjva-mdb-manifest.yml
else
  echo "app is running applying,is mongodb"
  kubectl -n jjva-ns-svc-pod apply -f jjva-mdb-manifest.yml
  echo "Here is pod ${mss_pod_app} latest deployed successfully"
fi
#!/bin/bash
